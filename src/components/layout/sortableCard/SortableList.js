...
export default class SortableList extends Component {
  ...
  componentDidMount() {
    //  this.props.updateRow(this.recalculate.bind(this));
    this._onUpdateLayouts();
  ...
  _onLayoutRow(rowKey, {nativeEvent: {layout}}) {
    ...
    const rowLayouts = {...this.state.rowsLayouts};
    if(rowLayouts[rowKey] && this.props.dynamicRowIndex.includes(rowKey)){
      const prevHeight = rowLayouts[rowKey].height;
      const heightDiff = layout.height - prevHeight;
      rowLayouts[rowKey] = layout;
      this.getPromises();
      this.setState({
        rowsLayouts: rowLayouts,
        contentHeight: this.state.contentHeight + heightDiff
      });
    }
  }
